...
const token = jwt.sign({ id: nuevoUsuario_id, email }, process.env.JWT_SECRET, { expiresIn: '1h' });
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: usuario._id, email }, process.env.JWT_SECRET || "clave", { expiresIn: "1h" });
